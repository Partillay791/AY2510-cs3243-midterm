## Week 1
OO has higher level mechanism than the procedural paradigm.  
Every object has an `interface` and `implementation`:
- `interface` -> other objects interact with it
- `implemetation` -> supports the interface  

Encapsulation -> `packaging` and `information hiding`  

Constructors are not inherited by subclasses. If a constructor doesn't explicitly invoke a superclass constructor, the Java
compiler automatically inserts a call to the no-argument constructor of the superclass. If the superclass does not have a
no-argument constructor, you will get a compile-time error.  

Having an abstract method makes you an abstract class!  

Interfaces can also contain `constants` and `static` methods.  

Dynamic binding -> `Overriden` methods, Static binding -> `Overloaded` methods

Polymorphism: `substitutability`, `operation overriding`, `dynamic binding`  

Exceptions: `Checked exceptions`, `Errors`, `Runtime exceptions`

## Week 2
### SDLC1: Sequential Model (Waterfall model)
Not good because had to state all requirements accurately.
### SDLC2: Iterative Model
1. Breadth-first -> evolve everything in parallel
2. Depth-first -> fleshing out only some components/functionality area  

Regression Testing -> re-testing of the software to detect regressions  
Semiautomatic testing of CLI app -> input/output redirection

## Week 3
Integration -> Combining parts of a software product  
Gradle, Maven -> build automation, dependency management tools  
CI -> integration, building, and testing happens automatically after each code change
- CD: changes not only integrated, but deployed to end-users at the same time  

Importing a package **does not** import its sub-packages.  
`Static` import -> import `static` members of a type.  

<img alt="img_1.png" height="244" src="../CS2103_fcs_img/access_modifier.png" width="449"/>

The earlier a bug is found, the `easier` and `cheaper` to have it fixed.  
A test driver is the code that `drives` the SUT for the purpose of testing.  
`JUnit` is a tool for automated testing of Java programs.  
Unit testing -> testing individual units (need to be done in isolation)  
Stubs can isolate the SUT from its dependencies.
- same interface, but simple implementation (unlikely to have bugs)

## Week 4
### Class diagrams (describe structure, not behaviour of OOP solution)
<img alt="img_1.png" height="200" src="../CS2103_fcs_img/uml_class_representation.png" width="560"/>

`Operations` and/or `attributes` can be omitted  
Visibility: `+` for public, `-` for private, `#` for protected, `~` for package private
- visibility has no default

Generic classes: a dotted rectangle at top right corner with texts such as `Element`, `? extends E`  
Class-level: `underline`  
Association: `solid line` or as an attribute like `name: type[multiplicity] = default value`
- Association labels -> describe meaning of association, e.g. `label |>`
- Association roles -> indicate the role, it is placed closer to the class that it belongs to
- Multiplicity -> placed closer to the class that it belongs to, `0..1` is optional, `*` is W
  - it has no default
- Navigability -> unidirectional or bidirectional, (two uni is not bi), use arrowheads  

Notes : additional information  
Inheritance: triangle and solid line  
Composition: `whole-part` relationship, solid diamond symbol  
Aggregation: `container-contained` relationship, hollow diamond  
Dependency: transient interaction, dashed arrow  
Abstract: *ClassName* or {abstract} and ClassName (2 lines)  
Method abstract: *method1()* or method1() {abstract} (1 line)  
Interface: triangle and dashed line  
Association classes: dashed line connected to an association link

![img_1.png](../CS2103_fcs_img/enumeration.png)

### Object diagram (shows an object structure at a given point of time)
![img_1.png](../CS2103_fcs_img/img_1.png)

No compartment for methods. Attributes *can* be omitted. Object name can be omitted. No multiplicities.  
Association: `solid line`  
Static analysis: analyse code without executing it (like CheckStyle, PMD, FindBugs)

## Week 5
Greenfield: from scratch, Brownfield: replace/update  
FR: what the system should do, NFR: constraints under which the system is developed & operated  
- NFR: Data, environment requirements, accessibility, security...

Gathering requirements: `Brainstorming`, `Product surveys`, `Observation`, `User surveys`, `Interviews`,
`Focus groups`, `Prototyping`  

Specifying requirements: `Prose`, `Feature Lists`, `User Stories`, `Glossary`, `Supplementary Requirements`

User stories: `As a ser, I can ... so that I can {benefit}`, high level -> `epic`
- convenient for `scoping`, `estimation`, and `scheduling`

Readability: `Avoid long methods`, `Avoid deep nesting`, `Avoid complicated expressions`, `Avoid magic numbers`,
`Make the code obvious`, `Do not trip up reader`, `Keep it simple, stupid`, `Avoid premature optimizations`, `SLAP`,
`Make happy path prominent`  

Error-Prone: `use default branch`, `no recycle variable/parameter`, `avoid empty catch blocks`, `delete dead code`,
`minimize scope of variables`, `minimize code duplication`  

Comments: `do not repeat obvious`, `write to reader`, `explain what, why not how`  

Refactoring: restructuring code in small steps w/o modifying external behaviour  

Assertions: define assumptions about the program state, use it liberally

## Week 6
Sequence diagrams -> model interactions between multiple entities.  
Entities: actors/components  
Activation Bar (optional): Period during which method is being executed  
Lifeline: instance is alive (dashed vertical line), `X` means die  
Invoke operation: solid filled arrow head  
Return (optional): dashed filled arrow head  
Classes: `:ClassName`  
Alt: no more than one alternative partitions be executed (can be none)
Method calls to static methods: add a `<<class>>` on top of the class name  
Ref frame: show as separate sequence diagram

![img_2.png](../CS2103_fcs_img/loop.png)
![img_1.png](../CS2103_fcs_img/alt.png)

![img_1.png](../CS2103_fcs_img/opt.png)
![img_1.png](../CS2103_fcs_img/par.png)
![img_1.png](../CS2103_fcs_img/ref.png)

Architecture diagrams -> free-form  
Debugging -> discovering defects  
Logging -> deliberate recording of certain info during prog exe for future ref (log file is black box)  

## Week 7
Use case -> A description of a set of sequences of actions, including variants, that a system
performs to yield an observable result of value to an actor
- Only describe externally visible behaviour
- Gives intention, not mechanics (omit UI details)
- Extensions numbering style is not a universal rule, only widely used
- Can include another use case, dotted arrow and an `<<include>>` notation
- Guarantee: specify promises


![img_1.png](../CS2103_fcs_img/use%20case%20example.png)

Software Design: Top-down, Bottom-up, Mix (none is better)  
Agile Design: expect design to change over product's lifetime  
Design Fundamentals: `Abstraction`, `Coupling`, `Cohesion`  
1. Abstraction: `data abstraction`, `control abstraction`
2. Coupling: measure of dependence
3. Cohesion: measure of how strongly-related and focused the various responsibilities of a component are

Integration approaches: `Late and one time` vs `Early and frequent`, `Big-bang` vs `incremental`  
Project planning: `milestones`, `buffers`, `issue trackers`, `work breakdown structure (WBS)`, `Gantt charts`  
Team structures: `egoless`, `chief programmer`, `strict-hierarchy`  
RCS: `Centralized (CRCS)`, `Distributed RCS (DRCS)`  
Workflow: `Forking`, `Feature branch`, `Centralized`

## Week 8
Testing: `Integration testing`, `System testing`, `Acceptance testing (for requirements)`,
`Alpha/Beta testing`, `Dogfooding`  
Approaches: `Scripted`, `Exploratory/Reactive/Erorr guessing/Attack-based/Bug hunting`  
Dependency injection: process of injecting objects to replace current dependencies with a different object  
Test coverage: metric used to measure the extent to which testing exercises the code, criteria:
1. Function/method coverage: based on functions executed
2. Statement coverage: based on LoC executed
3. Decision/branch coverage: based on decision points exercised
4. Condition coverage: based on boolean sub-expressions  

Test-Driven Development (TDD): advocates writing the tests before writing the SUT, while evolving functionality and
tests in small increments

## Week 9
Conceptual Class Diagrams (OODMs): captures class structures in the problem domain.  
Domain modelling via: `domain-specific notation`, `general purpose modelling notation`, `other`

### Activity Diagram (AD, model workflows)
1. start: solid point
2. end: solid point with a circle surround it
3. action: rounded-corners rectangle
4. flow: arrow heads
5. branch node: rhombus, with `[condition 1]`,`[condition 2]`
   1. multiple arrows can start from the same corner of a branch node
   2. `[Else]` can be omitted
6. merge node (optional): rhombus
7. fork node, join node: a bar
8. rake: give as a separate diagram
9. swimlane diagrams: partition an AD to show who is doing what

### Architecture Diagram
1. Minimize the variety of symbols
2. Avoid the discriminate use of double-headed arrows

### Design Principles
1. Separation of concerns principle (SoC): To achieve better modularity, separate the code into distinct sections,
such that each section addresses a separate concern.
   1. Higher cohesion, lower coupling
   2. Utilised by `n-tier architecture`
2. Single responsibility principle (SRP): A class should have one, and only one, reason to change.
3. Liskov substitution principle (LSP): Derived classes must be substitutable for their base classes.
4. Open-closed principle (OCP): A module should be open for extension but closed for modification.
That is, modules should be written so that they can be extended, without requiring them to be modified.
5. Law of Demeter (LoD):
   1. An object should have limited knowledge of another object.
   2. An object should only interact with objects that are closely related to it.

SOLID: SRP + OCP + LSP + Interface Segregation Principle (ISP) + Dependency Inversion Principle (DIP)

Agile processes:
1. eXtreme Programming (XP): stresses customer satisfaction
2. Scrum
   1. Scrum Master, Product Owner, Team
   2. Iterations -> Sprints
   3. Sprint -> preceded by planning meeting
   4. A key principle of Scrum is its recognition that during a project
   the customers can change their minds about what they want and need

### Developer Docs
Developer-to-developer documentation:
1. documentation for `developer-as-user` or;
2. documentation for `developer-as-maintainer`

There is a secret that needs to be understood in order to write good software documentation:
there isn’t one thing called documentation, there are four. They are:
`tutorials`, `how-to guides`, `explanation` and `technical reference`.  
Aim for comprehensibility:
1. Use plenty of diagrams
2. Use plenty of examples
3. Use simple & direct explanations
4. Get rid of statements tht do not add value
5. It is not a good idea to have separate sections for each type of artifact

When writing project documents, a top-down breadth-first explanation is easier to understand than a bottom-up one.  
Aim for 'just enough' developer documentation.  
Focus on providing higher level information that is not readily visible in the code or comments.  

## Week 10
Design pattern: An elegant reusable solution to a commonly recurring problem within a given context in software design.
Format: `Context`, `Problem`, `Solution`, `(O) Anti-patterns`, `(O) Consequences`, `(O) Other info`

Design patterns:
1. Singleton pattern
2. Facade pattern
3. Command pattern

Defensive programming:
1. Enforcing compulsory associations

Testing: Effective (find a lot of bugs) and efficient (find more bugs in less steps)

Test case design:
1. Positive vs Negative test case
2. Black box vs Glass box
   1. Black-box: specification-based or responsibility-based (specified external behaviour)
   2. White/Glass-box: structured or implementation-based
   3. Gray-box approach: test case design uses some important information about the implementation.
3. Based on use cases

Equivalence partitions: avoid testing too many, ensure all partitions tested

Data participants for EPs:
- the target object of the method call 
- input parameters of the method call 
- other data/objects accessed by the method such as global variables.
This category may not be applicable if using the black box approach
(because the test case designer using the black box approach will not
know how the method is implemented).

Boundary Value Analysis: more likely to find bugs in the EPs
- Three values: `below`, `in`, `above`

## Week 11
Design Patterns:
1. MVC: Model, View and Controller
2. Observer

Architectural Styles:
1. n-tier style: higher layers make use of services provided by lower layers
2. client-server style: at least one component playing the role of a server and
at least one client component accessing the services of the server
3. event-driven style: controls the flow of the application by detecting events
from event emitters and communicating those events to interested event consumers
4. transaction processing style: 
5. service-oriented style (SOA)

Most application use a mix.

Test input combination strategies:


