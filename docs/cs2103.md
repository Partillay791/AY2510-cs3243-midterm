...
Semiautomatic testing of CLI app -> input/output redirection (feed and compare with actual output)
...
`JUnit` is a tool for automated testing of Java programs. Unit testing -> testing individual units (need to be done in isolation)  

Stubs can isolate the SUT from its dependencies, i.e. same interface, but simple implementation (unlikely to have bugs)
...
Logging -> deliberate recording of certain info during prog exe for future ref (log file is black box of an airplane)  
...
- Only describe externally visible behaviour, not stuff like e.g. `LMS saves the file into the cache`
...
Software Design (product/external and implementation/internal): Top-down, Bottom-up, Mix (none is better)  
...
Approaches: `Scripted`, `Exploratory/Reactive/Erorr guessing/Attack-based/Bug hunting` (mix is good)  
